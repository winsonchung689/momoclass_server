...
public class LoginController {
		...
		try {
			...
			if(official_openid != null){
				...
				for(int j=0;j<official_list.length;j++){
					...
					String class_target_bak = class_number.split("_")[1];
					if(class_target_bak.equals("课后点评")){
						class_target_bak = "课评";
					}

					queryJson.getJSONObject("miniprogram").put("pagepath","/pages/detail/detail?openid=" + openid + "&studio=" + studio + "&comment_style=" + comment_style + "&role=" + role + "&class_target_bak=" + class_target_bak + "&duration=" + duration + "&date_time=" + date_time + "&student_name=" + student_name);
	...
	public String sendPaymentNotice(String openid,String amount,Integer days,String mark){
		...
		if("系统续费".equals(mark)){
			...
			// 通知管理员
			loginService.sendFeedback(Constants.admin_openid,studio,expired_time_new,days.toString(),mark);
			// 通知客户
			loginService.sendFeedback(openid,studio,expired_time_new,days.toString(),mark);
		}else{
			// 发提现通知给管理员
			String token = loginService.getToken("MOMO_OFFICIAL");
	...
	@ResponseBody
	public void sendOrderNotice(String openid,String goods_name){
		loginService.sendOrderNotice(openid,goods_name);
	}
	...
	@RequestMapping("/getCommunicateByStudent")
	@ResponseBody
	public List getCommunicateByStudent(String studio,String student_name,String openid){
		List list = null;
		try {
			List<User> list_user = dao.getUser(openid);
			String campus = list_user.get(0).getCampus();
			list = loginService.getCommunicateByStudent(studio,campus,student_name);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	public List getStudentByTeacher(String type,String openid,String duration_time,Integer page,String class_number,String campus){
		List list = null;
		try {
			list = loginService.getStudentByTeacher(type,openid,duration_time,page,class_number,campus);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	@RequestMapping("/getLessonByType")
	@ResponseBody
	public List getLessonByType(String openid,String type,String content){
		List list = null;
		try {
			list = loginService.getLessonByType(openid,type,content);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	@RequestMapping("/deleteDepartureRecord")
	@ResponseBody
	public int deleteDepartureRecord(Integer id){
		try {
			dao.deleteDepartureRecord(id);
		} catch (Exception e) {
			e.printStackTrace();
			return 0;
		}
...
//			throw new RuntimeException(e);
	...
	public int signUpSchedule(HttpServletRequest request, HttpServletResponse response){
		...
		if ("0".equals(package_id)) {
			...
			if(lessonPackages_mark.size()>0){
				for (int i = 0; i < lessonPackages_mark.size(); i++) {
					LessonPackage lessonPackage  = lessonPackages_mark.get(i);
	...
	public int updateDetailPhoto(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			if(list.size()>0){
				...
				String vuuid_get = message.getVuuid();
				if(!"noid".equals(vuuid)){
					vuuid_get = vuuid;
				}
				...
				if("课评".equals(class_target)){
					...
					dao.updateUuids(Integer.parseInt(id),studio,list_new.toString().replace(" ",""),vuuid_get);
				}
	...
	public int modifyLesson(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			if("阶段".equals(modifyHead)){
				...
				dao.updateSignUpCampus(studio,student_name,subject,campus,content);
...
//		String path = System.getProperty("user.dir");
	...
	@ResponseBody
	public ResponseEntity<byte[]> downloadFileByType(HttpServletRequest request, HttpServletResponse response) throws IOException{
		String file_name =  request.getParameter("file_name");
		String studio =  request.getParameter("studio");
		String type =  request.getParameter("type");
		studio = studio.replace("/","");
...
//			throw new RuntimeException(e);
	...
	public String insertUser(HttpServletRequest request, HttpServletResponse response){
		...
		if(avatarurl == null || avatarurl.isEmpty() || "undefined".equals(avatarurl)){
			avatarurl = "fa8a634a-40c2-412a-9a95-2bd8d5ba5675.png";
		}
			...
			if(lessons_amount>0 && "add_lessons".equals(modify_type)){
				...
				if("0".equals(package_id)){
					if("合并".equals(record_type)){
						...
						dao.updateSignUpPackageIdByStudent(Integer.parseInt(id),studio,campus,student_name,subject,10);
...
//			logger.error("update lesson error");
	...
	public String sendGift(HttpServletRequest request, HttpServletResponse response){
		...
		gift.setType(type);
		loginService.insertGift(gift);
		loginService.updateAddPoints(student_name,studio,-Math.round(Float.parseFloat(coins)),subject,campus,gift_name+"_"+gift_id,"兑换积分");
		return "push massage successfully";
	}
