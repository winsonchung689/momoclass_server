...
//			String campus = list_user.get(0).getCampus();
	...
	public List getLessonPackage(String student_name,String studio,String openid,String subject,String search_type,String duration_time){
		...
		try {
			List<User> list_user = dao.getUser(openid);
			String campus = list_user.get(0).getCampus();
			list = loginService.getLessonPackage(student_name,studio,campus,subject,search_type,duration_time);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	@ResponseBody
	public List getTipsDataUrl(String studio,Integer left_amount,String subject,String openid,String type){
		List list = null;
		try {
			List<User> list_user = dao.getUser(openid);
			String campus = list_user.get(0).getCampus();
			list = loginService.getTipsDataUrl(studio,left_amount,subject,campus,type);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
